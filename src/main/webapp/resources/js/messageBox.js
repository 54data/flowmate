$(document).ready(function() {
    ...
    $(document).on('click','.reply', function() {
        let receiverId = $(this).closest('.message').find('.sender-id').text().trim(); 
        
        let originalContent = $(this).closest('.message').find('.messageContext').text().trim();

        window.open(`/flowmate/message/messageSend?receiverId=${receiverId}&originalContent=${encodeURIComponent(originalContent)}`, '_blank', 'width=600, height=500, scrollbars=yes');
    });
    $(document).on('click','.md-reply', function() {
	    	let receiverId = $('.senderId').text().trim(); 
	    	
	    	let originalContent = $('.messageDetail').text().trim();	
		window.open(`/flowmate/message/messageSend?receiverId=${receiverId}&originalContent=${encodeURIComponent(originalContent)}`, '_blank', 'width=600, height=500, scrollbars=yes');
    });
